...
export const provideUniqueId=(tname,status)=>{
    return (req,res,next)=>{
        db.query(`SELECT * FROM ${tname} ORDER BY ID DESC LIMIT 1`,(err,data)=>{
            ...
            req.uid=status.substring(0,2).toUpperCase()+(new Date).getFullYear().toString().slice(-2)+'-'+id
            next()
        })
    }
}
export let addUsersInUsersTable=(tableName)=> {
    return (req,res,next)=>{
        next();
            ...
            if(!req.err){
                let d={username:req.body.username,password:password,status:req.body.status,Uid:req.uid}
...
export let updateUsersTable=(req,res, next)=>{
    ...
    db.query(`UPDATE Users SET ? WHERE uid=?`,[data,req.params.id],(err,data)=>{
